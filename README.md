...
- 7·8쪽의 A–D 접근 시나리오는 슬라이드 진입 시 자동으로 반복 재생됩니다. 삽입 완료 장면을 잠시 표시한 뒤 처음부터 반복하며, 별도 재생 버튼은 없습니다. 원문의 지게차·팔레트 이미지 요소와 D의 두 접근 대안을 사용합니다. 다른 슬라이드에서는 정지하며, 다시 진입하면 처음부터 재생합니다.

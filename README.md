...
UOS 공식 PowerPoint `UOS_PTformat_B`에서 옮긴 웹 디자인 시스템을 사용하는 **13장, 10분 45초 발표자료**입니다. 과제 요구 → 플랫폼 개조 → 인식·계획·삽입 제어 → A–D 검증 → 개발 일정 → 예상 주행 파이프라인 → 향후 추진 계획으로 구성했습니다.
...
- 7·8쪽의 A–D 접근 시나리오는 슬라이드 진입 시 자동 재생됩니다. 오른쪽 위 **일시정지 / 재생**, **다시 재생**으로 두 동작을 함께 제어합니다. 원문에 포함된 지게차·팔레트 이미지 요소를 사용하며, D의 두 접근 대안을 함께 표시합니다. 다른 슬라이드에서는 정지하며, 다시 진입하면 처음부터 재생합니다. 운영체제의 동작 줄이기가 설정된 경우에는 재생 버튼을 눌러 시작합니다.
...
- [`case-motion.js`](case-motion.js): 원문 이미지 요소의 자세 연결, 단계 표시 및 재생 제어.
- [`assets/approach-source.json`](assets/approach-source.json): 원문 이미지의 크기·자르기 범위, 위치·회전값 및 원본 파일 해시. 생성기가 HTML에 포함합니다.

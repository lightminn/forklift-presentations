...
REFERENCE = json.loads((ROOT/'assets/approach-source.json').read_text(encoding='utf-8'))

def motion_case(case, title, description, alternative=False):
    ref = REFERENCE['scenes'][case]
    x, y, angle = ref['poses']['start']
    vehicle = REFERENCE['vehicle']
    width, height = vehicle['width'], vehicle['height']
    crop = ' '.join(str(n) for n in vehicle['crop'])
    view = ' '.join(str(n) for n in ref['viewBox'])
    px, py, pw, ph = ref['pallet']
    obstacles = []
    for ox, oy, ow, oh, side in ref['obstacles']:
        label = f'<text x="{ox+ow/2}" y="{oy+oh/2+6}" text-anchor="middle">장애물</text>' if side == 'rear' else ''
        obstacles.append(f'<g class="motion-obstacle" data-obstacle="{side}"><rect x="{ox}" y="{oy}" width="{ow}" height="{oh}"/>{label}</g>')
    heading = title if alternative else f'<span>{case}</span>{title}'
    heading_class = 'motion-alternative-title' if alternative else 'case-title'
    return f'''<figure class="case motion-case" data-motion-case="{case}">
 <figcaption class="{heading_class}">{heading}</figcaption>
 <svg class="motion-scene" viewBox="{view}" role="img" aria-label="{case}: {description}">
  <title>{case} 접근 시나리오</title><desc>과제 원문의 지게차와 팔레트 이미지 요소를 사용한 {description}</desc>
{''.join(obstacles)}
  <image class="motion-pallet" href="assets/pallet-original.png" x="{px}" y="{py}" width="{pw}" height="{ph}"/>
  <g class="motion-vehicle" transform="translate({x} {y}) rotate({angle+180})">
   <svg x="{-width/2}" y="{-height/2}" width="{width}" height="{height}" viewBox="{crop}" preserveAspectRatio="none" overflow="hidden">
    <image href="assets/forklift-original.png" width="360" height="360"/>
   </svg>
  </g>
 </svg>
...
'''접근 방법은 과제에서 제시한 A부터 D까지의 네 가지 조건으로 나누어 개발한다. A는 팔레트가 지게차 정면에 있고 포크와 포켓의 방향이 일치하는 경우이다. 직진 접근과 저속 삽입을 수행하며, 좌우 위치 오차와 방향 오차 및 접촉 여부를 확인한다. B는 팔레트가 측면에 있으나 정렬에 필요한 접근 거리가 충분한 경우이다. 곡선 주행으로 방향을 맞춘 후 정지 없이 삽입하도록 경로를 생성한다. 개발 초기에는 A 조건에서 인식, 좌표 변환, 삽입 제어를 연결하여 시험한다. 이후 시작 위치를 좌우로 변경하여 B 조건을 평가한다. 이와 같이 시험 조건을 순차적으로 추가하여 삽입 제어와 접근 경로의 오류를 구분할 계획이다. 애니메이션은 과제 원문 8·9쪽에 배치된 지게차와 팔레트 이미지 요소를 그대로 사용한다. 시작 자세와 삽입 자세 및 B의 중간 자세를 기준으로 움직임을 연결하였다. 실제 차체에서 계산한 경로나 실험 결과를 나타내지 않는다.''',
[(BRIEF, '과제 원문 8·9쪽의 개별 이미지 요소와 배치·회전값을 사용한 애니메이션')], '출처: 과제 설명자료 8·9쪽 · 요구 동작')

add('주행 조건별 접근 방법 (2)', '07  주행 조건별 접근 방법 (2)', 60,
motion_pair('근거리 접근 및 후방 장애물 대응',
    motion_case('C', '근거리 접근', '후진 후 삽입') +
    '<div class="motion-d-group"><h3 class="case-title"><span>D</span>근거리 접근 · 후방 장애물</h3><div class="motion-d-options">' +
    motion_case('D1', '대안 1 · 회전 및 위치 변경', '회전 후 재접근', alternative=True) +
    motion_case('D2', '대안 2 · 제한 거리 후진', '후진 가능 범위 내 정렬', alternative=True) + '</div></div>', variants=True),
'''C는 팔레트와의 거리가 가까워 바로 정렬하기 어려운 경우이다. 우선 후진하여 필요한 공간을 확보한 후, 방향을 맞추어 다시 접근한다. D는 팔레트와의 거리가 가까우면서 후방에 장애물이 있는 경우이다. 이동 가능한 공간으로 위치를 변경한 뒤 재접근하거나, 장애물까지의 허용 거리 내에서 후진하여 정렬한다. 이때 제자리 회전 가능 여부는 실제 구동 구조에 따라 결정한다. 조향 차량은 전진과 후진을 조합하여 방향을 바꾸도록 계획한다. 차체 중심의 이동 경로뿐 아니라 돌출된 포크와 차체 후미의 충돌 가능성도 확인해야 한다. 이동 가능한 경로를 찾지 못하면 정지하도록 제어한다. 해당 정지는 안전 기능으로 평가하되 삽입 성공에는 포함하지 않는다. 애니메이션은 원문의 이미지 요소와 좌측·후방 장애물 배치를 유지하며, D의 회전·위치 변경과 제한 거리 후진을 두 대안으로 함께 보여 준다. 회전 동작의 적용 가능 여부와 실제 경로는 차체 선정 후 확인한다.''',
[(BRIEF, '과제 원문 10·11쪽의 개별 이미지 요소와 배치·회전값을 사용한 애니메이션')], '출처: 과제 설명자료 10·11쪽 · 요구 동작')
...
<script src="vendor/react.production.min.js"></script><script src="vendor/react-dom.production.min.js"></script><script src="./support.js"></script><script id="approach-reference" type="application/json">__APPROACH_REFERENCE__</script><script src="case-motion.js" defer></script></head>
...
''' + '\n\n'.join(sections) + '\n</x-import></x-dc></body></html>\n'
    html = html.replace('__APPROACH_REFERENCE__', json.dumps(REFERENCE, ensure_ascii=False, separators=(',', ':')))
    for name in ('case-motion.js', 'deck.css'):

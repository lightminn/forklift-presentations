...
renderer that produced each figure is named in the speaker notes.
"""
...
# ----------------------------------------------------------------- 1
add('개발 진행 보고', '3주차\n자율 지게차 개발', 20, '',
    """이번 주에는 팔레트의 두 포켓을 찾고, 위치와 포크 삽입 방향을 로봇 기준 좌표로 나타내는 기능을 구현하였다. 포켓은 포크가 들어가는 구멍이다. 차체가 아직 입고되지 않아 이번 주 결과는 모두 컴퓨터로 만든 장면에서 측정하였다. 평가 결과와 한계, 다음 작업을 보고한다.""",
    [(M2, '포켓 인식 검증 기록'), (E6, 'EPAL 6 캡처와 평가')], '측정 결과 보고')

# ----------------------------------------------------------------- 2
add('개발 진행 현황', '01  개발 진행 현황', 47, """
<h2 class="headline">과제 다섯 단계와 현재 진행 위치</h2>
<div class="pipeline grow"><article class="pipeline-step done"><h3>① 포켓 인식</h3><p>깊이 영상에서<br>팔레트·포켓 검출</p><p class="state">평가 완료</p></article><article class="pipeline-step done"><h3>② 좌표 산출</h3><p>포켓 위치·방향을<br>로봇 기준으로 표시</p><p class="state">구현 완료</p></article><article class="pipeline-step wait"><h3>③ 경로 생성</h3><p>장애물을 피하며<br>팔레트에 접근</p><p class="state">차체 입고 후</p></article><article class="pipeline-step now"><h3>④ 삽입·적재</h3><p>포켓 추적 후<br>포크 삽입</p><p class="state">추적: 다음 주<br>삽입: 차체 입고 후</p></article><article class="pipeline-step wait"><h3>⑤ 이송·하역</h3><p>목적지까지<br>운반·하역</p><p class="state">삽입 구현 후</p></article></div>
<div class="takeaway">이번 주: ①·② 구현 | 센서 관측 기록·재생 일치 확인</div>
""",
    """과제의 작업 절차는 포켓 인식부터 운반·하역까지 다섯 단계이다. 이번 주에는 1단계 포켓 인식과 2단계 포켓 위치·방향의 로봇 기준 좌표 산출을 구현하였다. 3단계 경로 생성과 실제 포크 삽입·적재는 차체의 주행 특성이 필요하므로 입고 뒤 착수할 예정이다. 4단계 중 차체 없이 시험할 수 있는 포켓 위치 추적은 다음 주에 시작한다. 시뮬레이터가 낸 센서 측정값을 기록한 뒤 새 프로세스에서 재생하여 같은 값이 읽히는 것을 확인하였다.""",
...
""",
    """2D LiDAR는 센서가 설치된 높이의 수평면에서 물체까지의 거리를 잰다. MuJoCo에서 설치 높이를 바닥 기준 0.50미터로 설정하여 수평 360도 방향을 측정하였다. 이 조건에서 벽·기둥·상자에서는 거리 측정점 298개가 나왔지만, 높이가 144밀리미터인 유럽형 EPAL 6 팔레트에서는 0개였다. 제작 예정인 축소 T11의 높이도 90밀리미터로 관측면보다 낮다. 그래서 현재 설계에서는 LiDAR를 주변 장애물 측정에, 컬러 영상과 깊이 정보를 함께 얻는 RGB-D 카메라를 팔레트 인식에 사용한다. 로봇 이동량을 계산하는 오도메트리가 없으므로 지도 작성과 로봇 위치 추정도 아직 구현하지 않았다.""",
...
{figure('05_two_specs.mp4', '같은 접근 자세 101개를 EPAL 6과 축소 T11 두 규격에 올리고 같은 코드로 검출한 비교 영상', '')}
...
<div class="takeaway">같은 코드 · 접근 자세 101개: EPAL 6 97개 · 축소 T11 97개 검출 | 별도 60자세 평가 최대 오차 509.1 mm</div>
""",
    """포켓을 판정하는 기준값을 정하는 방식으로 두 가지를 놓고 비교하였다. 첫째는 고정 기준값으로, 개발에 쓴 형상에서 잘 맞는 수를 골라 상수로 두는 방식이다. 이 방식은 규격이 다른 팔레트를 만나면 값을 다시 맞춰야 한다. 둘째는 치수 기반 산출로, 팔레트 규격에서 그때그때 계산하는 방식이다. 길이에 해당하는 값은 규격에 비례하게 하고, 필요한 측정점 수는 관측되는 면적에 비례하도록 하였다. 둘째 방식을 채택하였다. 영상은 비스듬한 위치에서 정렬하며 가까워지는 접근 자세 101개를 두 규격에 올리고 같은 코드로 처리한 결과이다. 왼쪽은 유럽 규격 EPAL 6, 오른쪽은 제작 예정인 축소 T11이며, 코드는 고치지 않고 각 규격의 치수만 넣었다. 두 규격 모두 101자세 중 97자세에서 포켓을 검출하였고, 검출된 자세의 위치 오차는 두 규격 모두 1센티미터 이내였다. 다만 자세를 더 넓게 흩어 놓은 별도 60자세 평가에서는 축소 T11에서 위치 오차가 509.1밀리미터인 사례가 한 건 있었다. 검출 수와 좌표 정확도는 구분해야 한다.""",
    [(WK3, '항별 분리 측정: 문턱만 고쳐도 50/101, 바닥 제외만 고치면 0/101'),
     ...
     (PLAN, '유도 공식과 기준점 선택'),
     (WK3, '접근 자세 101개 · 두 규격 모두 97개 검출, 최악 위치오차 6.1 / 4.8 mm')], '화면 생성: 측정 리그')
...
{figure('11_gazebo_approach.mp4', 'Gazebo 카메라 시점에서 포켓 검출 결과를 보인 접근', '', video=True, cls='')}
{figure('14_external_approach.mp4', 'MuJoCo 잠정 차체 모델로 같은 접근 자세를 밖에서 본 영상', '', video=True, cls='')}
</div>
<div class="takeaway">왼쪽: 포켓 검출 성공·실패 | 오른쪽: 같은 자세의 외부 시점</div>
""",
    """팔레트를 비스듬히 바라보다 정렬하며 가까워지는 자세를 순서대로 확인하였다. 카메라–팔레트 앞면 거리를 2.95미터에서 0.95미터까지 20밀리미터씩 바꾼 101개 자세 중, 왼쪽 Gazebo 장면에서는 97개에서 포켓을 검출하였다. 초록은 정답 위치, 자홍은 검출 위치이다. 오른쪽은 같은 자세를 잠정 차체 모델 밖에서 보여 주며 주황색 선으로 카메라가 볼 수 있는 범위를 표시한다. 오른쪽에는 프레임별 판정을 표시하지 않았으므로 검출 결과는 왼쪽 영상에서만 읽는다.""",
    [(E6, 'Gazebo 촬영 파이프라인'), (PLAN, '자세 범위와 검출 판정')], '화면 생성: Gazebo Harmonic · MuJoCo')

# ----------------------------------------------------------------- 9
add('평가 결과', '07  평가 결과', 70, f"""
<h2 class="headline">EPAL 6 장면 100개의 포켓 인식 평가</h2>
...
<div class="takeaway">p95: 오차 95 %가 이 값 이하 | 실센서 깊이 오차·축소 T11 미반영</div>
""",
    """EPAL 6 모델로 만든 장면 100개를 평가하였다. 포켓을 검출해야 하는 장면 60개는 모두 검출하였고, 검출 대상이 아닌 40개에서는 잘못된 검출이 0개였다. 검출 대상 60개에는 개발용 42개와 별도로 나눈 평가용 18개가 포함된다. 포켓 위치 오차 p95는 8.17밀리미터, 방향 오차 p95는 0.053도였다. p95는 오차의 95퍼센트가 해당 값 이하라는 뜻이다. 수치상 초기 개발 로드맵의 제안 목표인 검출률 95퍼센트 이상, 위치 오차 20밀리미터 이하, 방향 오차 2도 이하에 들었다. 다만 실제 센서의 깊이 오차는 거리 2.5미터에서 약 50밀리미터로 알려져 있고, 이는 방금 보고한 위치 오차 8.17밀리미터보다 크다. 이 오차를 평가에 넣지 않았고, 축소 T11도 이 장면 세트에는 포함하지 않았다.""",
...
""",
    """앞 장은 설치 높이에 따른 차이였다. 이번에는 현재 평가 기준인 0.50미터 높이에서 검출이 끊기는 이유를 확인하였다. 카메라 화면의 아래 경계는 전방 0.97미터 지점에서 바닥과 만난다. 그보다 가까운 바닥은 화면에 들어오지 않는다. 팔레트 앞면은 높이가 있어 위쪽 일부가 조금 더 가까이까지 남지만, 포켓 입구는 아래쪽이라 바닥과 함께 화면에서 밀려난다. 거리마다 따로 만든 정면 장면으로 확인하면, 카메라–팔레트 앞면 거리 1.20미터에서는 앞면과 포켓을 검출했지만 0.60미터에서는 윗판만 보이고 포켓은 검출하지 못했다. 따라서 매 영상에서 새로 포켓을 찾는 기능만으로는 삽입 구간의 위치 정보를 유지할 수 없다. 다음 주에는 이전에 검출한 위치를 추적하고, 관측이 끊겼을 때 삽입을 정지하는 조건을 시험할 계획이다.""",
...
{figure('09_docking_preview.mp4', '접촉과 화물 하중을 반영하지 않은 MuJoCo 잠정 차체의 포크 삽입 시각화', '', video=True, cls='')}
<div class="roadmap"><article class="done"><h3>이번 주까지</h3><p>포켓 인식·좌표 산출<br>장면 평가</p><p class="state">완료</p></article><article class="now"><h3>다음 주</h3><p>포켓 위치 추적<br>관측 중단 시 정지<br>주행 중 장애물 정지·우회</p><p class="state">차체 없이 시험</p></article><article class="wait"><h3>차체 입고 후</h3><p>차체 실측·시험용 팔레트 제작<br>실센서 재측정</p><p class="state">계획</p></article><article class="wait"><h3>그 이후</h3><p>지도·주행 제어<br>실제 삽입·이송·하역</p><p class="state">계획</p></article></div>
</div>
<div class="takeaway">이번 주 수치는 카메라 모형으로 낸 인식 결과</div>
""",
    """다음 주에는 세 가지를 시험할 계획이다. 첫째는 앞서 찾은 포켓 위치를 이어서 추적하는 기능이다. 앞 장에서 본 것처럼 가까워지면 앞면이 화면 밖으로 나가므로, 매 영상에서 새로 찾는 방식만으로는 삽입 구간을 버틸 수 없다. 둘째는 관측이 끊겼을 때 삽입을 정지하는 조건이다. 셋째는 주행 중에 사람이나 물체가 가까이 들어왔을 때의 정지와 우회이다. 계획한 경로를 따라 차체가 지나갈 통로 안에서 남은 거리를 보고, 여유가 모자라면 정지한다. 정지한 뒤 다시 출발할지는 관측이 회복된 다음에 판단한다. 다만 현재 2D LiDAR는 설치 높이의 수평면만 보므로 그보다 낮은 물체는 관측되지 않는다. 이 한계는 계획서에 적어 두었고 실제 관측 범위는 차체가 입고된 뒤에 측정한다. 차체 입고 후에는 치수와 카메라 위치를 실측하고 축소 T11을 제작해 실제 센서로 평가하겠다. 이후 지도 작성, 주행 제어, 실제 삽입·이송·하역을 진행한다.""",

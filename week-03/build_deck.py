...
# ----------------------------------------------------------------- 1
add('개발 진행 보고', '3주차\n자율 지게차 개발', 20, '',
    """이번 주에는 팔레트의 두 포켓을 찾고, 위치와 포크 삽입 방향을 로봇 기준 좌표로 나타내는 기능을 구현하였다. 포켓은 포크가 들어가는 구멍이다. 차체가 아직 입고되지 않아 이번 주 결과는 모두 시뮬레이션에서 측정하였다. 평가 결과와 한계, 다음 작업을 보고한다.""",
...
""",
    """팔레트를 비스듬히 바라보다 정렬하며 가까워지는 자세를 순서대로 확인하였다. 카메라–팔레트 앞면 거리를 2.95미터에서 0.95미터까지 20밀리미터씩 바꾼 101개 자세 중, 왼쪽 Gazebo 장면에서는 97개에서 포켓을 검출하였다. 초록은 정답 위치, 자홍은 검출 위치이다. 오른쪽은 같은 자세를 잠정 차체 모델 밖에서 보여 주며 주황색 선으로 카메라가 볼 수 있는 범위를 표시한다. 오른쪽에는 프레임별 판정을 표시하지 않는다. 검출 결과는 왼쪽 영상에서만 읽는다. 두 영상 모두 실제 주행이나 경로 제어 시연이 아니다.""",
...
<div class="takeaway">설치 높이 0.27 / 0.50 / 0.90 m 에서 검출 22 / 20 / 4 | 실제 설치 높이 미확정</div>
""",
...
""",
    """앞 장은 설치 높이에 따른 차이였다. 이번에는 현재 평가 기준인 0.50미터 높이에서 검출이 끊기는 이유를 확인하였다. 카메라 화면의 아래 경계는 전방 0.97미터 지점에서 바닥과 만난다. 그보다 가까운 바닥은 화면에 들어오지 않는다. 팔레트 앞면은 높이가 있어 위쪽 일부가 조금 더 가까이까지 남지만, 포켓 입구는 아래쪽이라 바닥과 함께 화면에서 밀려난다. 측정 리그의 정면 장면에서 카메라–팔레트 앞면 거리 1.20미터에서는 앞면과 포켓을 검출했지만, 0.60미터에서는 윗판만 보이고 포켓은 검출하지 못했다. 두 거리는 각각 따로 만든 정지 장면이며, 삽입 중 연속 관측을 시험한 것은 아니다. 따라서 매 영상에서 새로 포켓을 찾는 기능만으로는 삽입 구간의 위치 정보를 유지할 수 없다. 다음 주에는 이전에 검출한 위치를 추적하고, 관측이 끊겼을 때 삽입을 정지하는 조건을 시험할 계획이다.""",
...
<div class="roadmap"><article class="done"><h3>이번 주까지</h3><p>포켓 인식·좌표 산출<br>장면 평가</p><p class="state">완료</p></article><article class="now"><h3>다음 주</h3><p>포켓 위치 추적<br>관측 중단 시 정지<br>주행 중 장애물 정지·우회</p><p class="state">시험 예정</p></article><article class="wait"><h3>차체 입고 후</h3><p>차체 실측·시험용 팔레트 제작<br>실센서 재측정</p><p class="state">계획</p></article><article class="wait"><h3>그 이후</h3><p>지도·주행 제어<br>실제 삽입·이송·하역</p><p class="state">계획</p></article></div>
</div>
<div class="takeaway">현재 수치는 실제 센서·차체로 검증한 값이 아님 | 차체 입고 후 재측정</div>
""",

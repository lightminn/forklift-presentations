...
<div class="pipeline grow"><article class="pipeline-step done"><h3>① 팔레트 인식</h3><p>포크 넣을 구멍 찾기</p><p class="state">구현·평가 완료</p></article><article class="pipeline-step done"><h3>② 로봇 기준 좌표</h3><p>구멍의 방향·거리 계산</p><p class="state">구현 완료</p></article><article class="pipeline-step wait"><h3>③ 접근 경로</h3><p>장애물 피해 접근</p><p class="state">차체 입고 후</p></article><article class="pipeline-step now"><h3>④ 추적·삽입</h3><p>보면서 포크 넣기</p><p class="state">다음 주 착수</p></article><article class="pipeline-step wait"><h3>⑤ 이송·하역</h3><p>옮기고 내려놓기</p><p class="state">이후</p></article></div>
<div class="takeaway">3단계부터는 차체를 받은 뒤</div>
""",
...
{figure('21_lidar_plane.png', '옆에서 본 관측 평면과 그 높이에서 360도 방향의 반사 지점. 팔레트 반사는 0개', '설치 높이 0.50 m · MuJoCo')}
<div class="takeaway">LiDAR는 벽·장애물, 카메라는 팔레트</div>
""",
...
{figure('20_pipeline_stages.png', '깊이 영상, 작업 영역, 평면 후보, 기둥·빈칸 격자, 포켓 자세의 다섯 단계', 'Gazebo 촬영 장면 · 단계별 중간 결과')}
<div class="takeaway">기둥·빈 칸·위아래 판이 함께 보여야 팔레트로 인정</div>
""",
...
<div class="takeaway">T11 × 0.6 · 660 × 660 × 90 mm 제작</div>
""",
...
<h2 class="headline">버린 점 안에 판별 근거가 있었다</h2>
<p class="small muted">바닥에서 반사된 점이 팔레트 점과 섞이므로 일정 높이 아래는 버린다 · 그 높이가 20 mm로 고정돼 있었고 팔레트 바닥판은 22 mm다</p>
{figure('22_floor_cut.png', '촬영한 팔레트 화면에 버리는 구간을 색으로 칠한 비교. 왼쪽은 바닥판이 그 안에 들어간다', '색칠한 띠가 버려지는 구간 · 노란 선이 바닥판 윗면')}
<div class="takeaway">시험 자세 125개 중 12개만 검출</div>
""",
...
{figure('04_frozen_vs_derived.mp4', '촬영한 같은 주행을 두 설정으로 나란히 재생. 왼쪽은 버리는 띠가 두꺼워 끝까지 못 찾는다', '색 띠가 버리는 구간 · 촬영한 같은 주행 101장 중 좌 0장, 우 97장 검출')}
<div class="rule body"><b>전에는 모든 팔레트에 같은 값</b>을 썼고, <b>이제는 그 팔레트의 치수로 계산</b>한다<br>바닥에서 버리는 높이 20 → 7.3 mm · 면으로 인정할 두께 20 → 7.8 mm · 팔레트로 인정할 최소 측정점 100 → 15</div>
<div class="takeaway">축소 T11도 16개 → 59개 검출</div>
""",
    """앞 장의 고정된 기준값을 팔레트 치수에서 계산하도록 변경하였다. 길이 기준은 팔레트를 줄인 비율만큼 줄인다. 측정점 개수 기준은 보이는 면적에 맞추므로, 길이를 줄인 비율을 두 번 곱하여 적용한다. 바닥 부근의 측정점을 제거하는 높이는 바닥판 두께의 3분의 1로 정하였다. 화면은 촬영한 접근 주행 101장에 두 설정을 각각 적용한 것이다. 앞 장에서 본 버리는 구간을 색으로 칠해 두었다. 왼쪽은 그 띠가 두꺼워 바닥판을 덮고, 101장 모두에서 포켓을 찾지 못한다. 오른쪽은 띠가 얇아 바닥판이 남고, 97장에서 두 포켓을 찾는다. 아래 띠는 장면마다의 성공 여부이다. 팔레트 규격이 바뀌면 코드를 고치는 대신 새 치수를 입력할 수 있다. 제작 예정인 축소 T11 모델도 시험 자세 60개 중 검출한 자세가 16개에서 59개로 늘었다. 제작 후에는 실제로 잰 치수를 입력하여 기준값을 다시 계산한다.""",
...
<div class="takeaway">101개 자세 중 97개 검출</div>
""",
...
{figure('03_scene_variety.png', '먼 거리, 가까운 거리, 비스듬한 자세, 가림, 유사 물체, 팔레트 없음', '100장면 중 6가지 조건 · Gazebo', cls='')}
{figure('17_targets.png', '검출률 100%, 위치 오차 p95 8.17 mm, 방향 오차 p95 0.053도', '로드맵의 초기 목표와 이번 측정값', cls='')}
</div>
<div class="takeaway">가상 장면 기준 · 실제 센서는 아직</div>
""",
...
<h2 class="headline">높일수록 가까운 데를 먼저 잃는다</h2>
{figure('13_camera_mount.png', '카메라 높이 0.27, 0.50, 0.90 m에서의 화각과 가장 가까운 검출 거리', '적색 카메라 · 주황 시야 · MuJoCo')}
<div class="takeaway">마스트 상단은 1.75 m 이내를 못 본다</div>
""",
...
<h2 class="headline">삽입 직전엔 앞면이 안 보인다</h2>
<div class="split grow" style="grid-template-columns:1fr 1fr">
{figure('06_near_field_blind.png', '0.60 m에서 전면이 사라지고 상판만 보이는 화면', '0.60 m · 앞면 소실 · 측정 리그', cls='')}
{figure('09_docking_preview.mp4', '차체 모델이 팔레트에 포크를 넣는 미리보기', '포크가 들어가는 구간 · MuJoCo', video=True, cls='')}
</div>
<div class="takeaway">포크는 420 mm · 인식은 0.8 m에서 끊긴다</div>
""",
...
<div class="takeaway">가상 조건 값 · 실제 센서로 재측정</div>
""",

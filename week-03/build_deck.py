...
<h2 class="headline">20 mm 고정 기준에서 팔레트 바닥판도 제거</h2>
<p class="small muted">바닥 잡점을 제거하려고 바닥에서 일정 높이까지의 측정점을 제외하였다</p>
{figure('22_floor_cut.png', '촬영 화면에 제외 구간을 표시한 비교. 좌측은 바닥판이 제외 구간에 포함된다', '')}
<div class="takeaway">초기 EPAL 6 합성 시험: 팔레트 자세 125개 중 12개에서 포켓 검출</div>
""",
    """초기 검출기는 바닥에서 생기는 잡점을 줄이려고 바닥 기준 20밀리미터까지의 측정점을 일괄 제외하였다. 초기 팔레트의 바닥판은 50밀리미터 두께여서 하단 형상이 남았지만, EPAL 6은 22밀리미터, 제작 예정인 축소 T11은 15밀리미터이다. 같은 기준을 적용하면 잡점과 함께 포켓을 구분하는 데 필요한 바닥판도 사라진다. 이 설정에서는 EPAL 6 모델의 위치와 방향을 바꾼 합성 자세 125개 중 12개에서만 포켓을 찾았다. 그림은 동일한 장면에서 20밀리미터 고정 기준과 팔레트 치수에 맞춘 기준이 어떤 측정점을 남기는지 비교한 결과이다.""",
...
<div class="swap"><div><b>바닥에서 제외하는 높이</b><span>20 mm <i>→</i> 7.3 mm</span></div><div><b>같은 면으로 묶는 두께</b><span>20 mm <i>→</i> 7.8 mm</span></div><div><b>팔레트로 인정할 최소 점 수</b><span>100개 <i>→</i> 15개</span></div></div>
<div class="takeaway">축소 T11 합성 자세: 0/60 → 58/60 검출 | 최대 위치 오차 509.1 mm 1건</div>
""",
    """고정된 숫자 대신 팔레트 치수에서 판정 기준을 계산하도록 변경하였다. 바닥 측정점을 버리는 높이는 바닥판 두께의 3분의 1로 줄였다. 길이 기준은 축소 비율에 맞추고, 필요한 측정점 수는 관측 면적에 맞춰 축소 비율의 제곱을 적용하였다. 영상은 동일한 Gazebo 합성 접근 자세 101개를 두 설정으로 처리한 결과이다. 고정 기준에서는 포켓 검출이 0개였고, 치수 기반 기준에서는 97개였다. 제작 예정인 축소 T11 모델의 별도 합성 자세 60개에서도 검출 수가 0개에서 58개로 늘었다. 다만 검출된 자세 중 위치 오차가 509.1밀리미터인 사례가 한 건 있었다. 따라서 검출 수가 늘었다는 결과와 좌표 정확도는 구분해야 한다.""",
...
{figure('14_external_approach.mp4', 'MuJoCo 잠정 차체 모델로 같은 접근 자세를 밖에서 보고 카메라 시야를 표시한 영상', '잠정 차체의 카메라 시야 참고 영상 · MuJoCo', video=True, cls='')}
</div>
<div class="takeaway">왼쪽: 101자세 중 97개 검출 | 오른쪽: 잠정 차체의 카메라 시야 참고 | 실제 주행 아님</div>
""",
    """팔레트를 비스듬히 바라보다 정렬하며 가까워지는 합성 자세를 순서대로 확인하였다. 카메라–팔레트 앞면 거리를 2.95미터에서 0.95미터까지 20밀리미터씩 바꾼 101개 자세 중, 왼쪽 Gazebo 장면에서는 97개에서 포켓을 검출하였다. 초록은 정답 위치, 자홍은 검출 위치이다. 오른쪽은 같은 자세를 잠정 차체 모델 밖에서 보여 주며 주황색 선으로 카메라가 볼 수 있는 범위를 표시한다. 오른쪽의 ‘카메라 시야에 들어옴’은 검출 판정이 아니므로 왼쪽 검출 수와 직접 비교할 수 없다. 두 영상 모두 실제 주행이나 경로 제어 시연이 아니다.""",
...
<div class="takeaway">합성 비교: 검출 최근접 거리 1.05 / 1.25 / 1.75 m | 실제 설치 높이 미확정</div>
""",
    """카메라 높이가 근거리 포켓 검출에 미치는 영향을 합성 환경에서 비교하였다. 바닥 기준 높이 0.27, 0.50, 0.90미터에서 포켓을 검출한 가장 가까운 카메라–팔레트 전면 거리는 각각 1.05, 1.25, 1.75미터였다. 검출이 끊긴 순간 포크 끝에서 팔레트 전면까지는 각각 850, 1,050, 1,550밀리미터가 남았다. 같은 정면 자세 46개 중 검출된 자세는 각각 21, 18, 10개였다. 이번 조건에서는 카메라가 높을수록 포켓을 가까운 거리까지 보기 어려웠다. 0.90미터는 과제 설명자료의 마스트 상단 설치안이며, 마스트는 포크를 올리는 수직 구조물이다. 실제 카메라 위치는 차체가 입고된 뒤 실측하여 결정할 예정이다.""",
    [(E6, '장착 높이별 검출 구간 측정'),
...
""",
    """앞 장은 설치 높이에 따른 차이였다. 이번에는 현재 합성 평가 기준인 0.50미터 높이에서 검출이 끊기는 이유를 확인하였다. 카메라 화면의 아래 경계는 전방 0.97미터 지점에서 바닥과 만난다. 그보다 가까운 바닥과 바닥에 붙은 팔레트 앞면은 화면에 들어오지 않는다. 측정 리그의 정면 장면에서 카메라–팔레트 전면 거리 1.20미터에서는 앞면과 포켓을 검출했지만, 0.60미터에서는 윗판만 보이고 포켓은 검출하지 못했다. 따라서 매 영상에서 새로 포켓을 찾는 기능만으로는 삽입 구간의 위치 정보를 유지할 수 없다. 다음 주에는 이전에 검출한 위치를 추적하고, 관측이 끊겼을 때 삽입을 정지하는 조건을 합성 환경에서 시험할 계획이다. 이 결과는 실제 카메라 영상이나 실제 삽입 주행이 아니다.""",

...
<h2 class="headline">바닥 잡음을 지우는 기준이 판별에 쓰는 바닥판까지 제거</h2>
{figure('18_floor_cut.png', '고정값 20 mm 절단선이 두 형상의 바닥판을 지우는 단면도', '팔레트 하부 단면 · 세로 실척 · 적색은 고정값 20 mm 가 제거하던 구간')}
<div class="rule body">검출기는 바닥 근처 점을 잡음으로 버린다 · 그 경계 20 mm 안에 판별용 바닥판 22 mm·15 mm 가 들어 있었다</div>
<div class="takeaway">결과: 실물 규격 자세 125개 중 12개만 유효 · 정면 자세는 전 거리 미검출</div>
""",
...
<h2 class="headline">팔레트 치수로부터 검출 기준값을 계산</h2>
{figure('04_frozen_vs_derived.mp4', '같은 주행을 종전 고정값과 규격 기반 산출값으로 나란히 재생', '동일 주행 183 프레임을 두 설정으로 실행 · 좌: 종전 고정값, 전 구간 미검출 · 우: 규격에서 산출, 150 프레임 검출 · 하단 띠는 프레임별 검출 여부')}
<div class="rule body">팔레트가 작아지면 길이 기준은 같은 비율로, 점 개수 기준은 면적이므로 비율의 제곱으로 축소</div>
<div class="takeaway">효과: 팔레트 규격만 입력하면 표준 규격과 축소 T11 모두 검출 · 축소 T11 시험 자세 60개 중 16개 → 59개</div>
""",

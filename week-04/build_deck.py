...
{figure('10_mission_full.mp4', '위에서 내려다본 임무 전체 영상. 왼쪽 위 작은 화면은 지게차 카메라, 오른쪽 위에 현재 단계 표시', '', cls='')}
...
""",
    """임무 전체를 위에서 내려다본 영상을 2배속으로 보인다. 오른쪽 위 표시가 현재 단계이고, 왼쪽 위 작은 화면은 지게차 카메라가 보는 장면이다. 지게차는 관측 지점에서 팔레트를 찾은 뒤 계획한 경로를 따라 접근해 포크를 넣고 들어 올린다. 팔레트를 든 채 후진해 목적지 쪽으로 옮긴 뒤 앞으로 들어가 초록 원 안에 내려놓고, 포크를 빼며 물러난다. 오른쪽 표처럼 카메라가 추정하는 것은 팔레트 위치이고, 로봇 위치와 장애물, 목적지는 시뮬레이터 값을 쓴다. 이 부분은 실물 단계에서 2D LiDAR로 바꾼다.""",
    [(VIEWS, '임무 전체 영상 — 사용자 제공 Isaac Sim 녹화(2026-09-23), 단계 표시는 영상을 보고 붙임'), (STATUS, '삽입 깊이 360 mm와 사용 입력의 구분')],
    '화면 생성: 사용자 제공 Isaac Sim 녹화 · 2배속 · 단계 표시는 영상 판독')

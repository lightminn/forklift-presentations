...
2026-09-09 보완: 발표자료 7·8쪽의 정적 A–D 그림을 편집 가능한 SVG와 `case-motion.js`의 애니메이션으로 교체하였다. 위 원본 그림은 출처 확인용으로 보존하였다. A의 직선 접근, B의 곡선 정렬과 연속 진입, C의 후진 후 재접근, D의 제한 후진 후 재접근을 상면도로 재구성하였다. D는 원문에 제시된 두 대안 중 제한 거리 내 후진을 표현한다. 애니메이션의 좌표·속도·회전반경은 동작 설명을 위한 값이며, 실제 차체의 계획 경로나 로봇 시험 결과가 아니다.
